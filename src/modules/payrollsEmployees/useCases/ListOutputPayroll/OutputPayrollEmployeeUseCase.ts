    ...
    async execute(id: string, year: number, month: string, user_id: string) {
        ...
        payrolls2.map((payroll) =>{
         ...
         let employeePayroll: ICreatePayrollEmployeeDTO = {
            ...
            subsidy_shift: payroll.subsidy_shift ?? 0,
            subsidy_night: payroll.subsidy_night ?? 0,
            subsidy_risk: payroll.subsidy_risk ?? 0,
            subsidy_attendance: payroll.subsidy_attendance ?? 0,
            subsidy_performance: payroll.subsidy_performance ?? 0,
            subsidy_leadership: payroll.subsidy_leadership ?? 0,
            subsidy_commission: payroll.subsidy_commission ?? 0,
            salary_thirteenth: payroll.salary_thirteenth ?? 0,
            salary_fourteenth: payroll.salary_fourteenth ?? 0,
