...
class CreatePayrollUseCase {
    ...
    async execute(month: string, year: number, user_id: string) {
        ...
        const payrollMouth = await this.payrollRepository.findByMouth(month!, user.company_id)
        const payrollYear = await this.payrollRepository.findByYear(year!, user.company_id)
